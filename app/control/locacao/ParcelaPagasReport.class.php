<?php
/**
 * ParcelaPagasReport Report
 * @author  <your name here>
 */
class ParcelaPagasReport extends TPage
{
    protected $form; // form
    protected $notebook;
    ...
    function __construct()
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_ParcelaPagasReport');
        $this->form->class = 'tform'; // change CSS class        
        $this->form->style = 'display: table;width:100%'; // change style
        //$this->form->setFormTitle('ParcelaPagasReport Report'); 
        ...
        $dtpagtode->addValidation('Pagas de', new TRequiredValidator);
        $dtpagtode->addValidation('Pagas de', new TDateValidator);
        $dtpagtode->addValidation('Pagas até', new TRequiredValidator);
        $dtpagtode->addValidation('Pagas até', new TDateValidator);
        ...
        // add the fields
        $this->form->addQuickField('Pagas de', $dtpagtode, 100);
        $this->form->addQuickField('Pagas até', $dtpagtoate, 100);
    ...
    function onGenerate()
    {
        ...
        {            
            ...
            {   
                ...
                $designer->setTitle('Relação de Parcelas Pagas ao Locador');
            	...
            	if (!file_exists("app/output/ParcelaPagasReport.{$format}") OR is_writable("app/output/ParcelaPagasReport.{$format}"))
                {
                    $designer->save("app/output/ParcelaPagasReport.{$format}");
                }
                else
                {
                    throw new Exception(_t('Permission denied') . ': ' . "app/output/ParcelaPagasReport.{$format}");
                }
                
                // open the report file
                parent::openFile("app/output/ParcelaPagasReport.{$format}");
            }
            else
            {
